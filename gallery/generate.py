...
"""Generate the bounded deterministic Trophy Manager gallery."""
...
def placements_tsv() -> bytes:
    lines = ["case_id\tlabel\tx\ty\tz\tblock_state\tnbt\texpected"]
    lines.extend(
        "\t".join(
            (
                placement.case_id,
...
def verify_function() -> bytes:
    ...
    for placement in cases.PLACEMENTS:
        ...
        if placement.nbt is not None:
            lines.append(
                f"execute unless data block {placement.x} {placement.y} "
                f"{placement.z} {placement.nbt} run tellraw @a "
                f"{{\"text\":\"gallery trophy-data mismatch: "
                f"{placement.case_id}\",\"color\":\"red\"}}"
            )
    lines.append(
        "tellraw @a {\"text\":\"Trophy Manager gallery verification "
        "finished; inspect any mismatch above\",\"color\":\"aqua\"}"
    )
    return text_bytes("\n".join(lines))
...
def base_files() -> dict[Path, bytes]:
    ...
    return {
        ...
        Path("datapack/pack.mcmeta"): json_bytes(
            {
                "pack": {
                    "description": "ATM 1.2.0 Trophy Manager review gallery",
                    "pack_format": 48,
                }
            }
        ),
        ...
        function_root / "load.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            "tellraw @a {\"text\":\"Trophy Manager review gallery loaded; "
            f"run /function {cases.NAMESPACE}:build\",\"color\":\"aqua\"}}"
        ),
